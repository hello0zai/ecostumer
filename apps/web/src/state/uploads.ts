...
// // Habilita o uso de Map e Set com Immer
// enableMapSet()
...
// export const areUploadsEmptyAtom = atom((get) => get(uploadsAtom).size === 0)
...
//     if (!upload) {
//       throw new Error(`Upload com ID ${uploadId} não encontrado.`)
//     }
...
//   const percentageSum = actions.reduce(
//     (acc, action) => acc + action.progress,
//     0,
//   )
//   return percentageSum / actions.length
// }
...
//   // Iniciar upload de vídeo e adicionar à fila de conversão de áudio
//   set(startVideoUploadAtom, uploadId)
//   set(addToAudioConversionQueueAtom, uploadId)
...
//     if (!get(isRunningAudioConversionAtom)) {
//       set(runAudioConversionQueueAtom)
//     }
//   },
// )
...
//     try {
//       const { url: uploadUrl } = await axios.get(
//         `/api/video-upload-url/${uploadId}`,
//       )
//       await axios.put(uploadUrl, upload.file, {
//         signal: abortController.signal,
...
//       // Simular a conversão de vídeo para MP3
//       setTimeout(() => {
//         set(
//           uploadsAtom,
//           createUpdateUploadDraft(uploadId, {
...
//     try {
//       const upload = get(getUploadById(uploadId))
//       const { audioFile } = upload

//       if (!audioFile) throw new Error('Arquivo de áudio não encontrado.')

//       const { url: uploadUrl } = await axios.get(
//         `/api/audio-upload-url/${uploadId}`,
//       )
...
//  */
// const fakeFile = new File(['dummy content'], 'example.mp4', {
//   type: 'video/mp4',
// })

// set(addUploadAtom, fakeFile)
